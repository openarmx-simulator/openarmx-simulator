...
const PHRASE_DB = {
  ...
  // ── 역동 구: 큰 좌우 움직임 ─────────────────────────────────
  // PH-013: 한 팔씩 번갈아 극단적으로 높이 올림 (L1 ±1.85~1.95, 비대칭)
  'PH-013': { name: '대형 좌우 교차',   poses: ['P-011','P-012','P-023','P-024','P-011','P-012','P-027','P-028'], dur: 0.65 },
  // PH-014: 한 쪽으로 팍! 치고 → 중앙 → 반대로 팍! (풍차 느낌)
  'PH-014': { name: '풍차 돌리기',      poses: ['P-023','P-002','P-024','P-002','P-027','P-002','P-028','P-002'], dur: 0.5  },
  // PH-015: 좌우 번갈아 사선 스윙 (중간 → 큰 비대칭 점층)
  'PH-015': { name: '사선 교차 스윙',   poses: ['P-019','P-020','P-023','P-024','P-019','P-020','P-023','P-024'], dur: 0.6  },

  // ── 역동 구: 큰 상하 움직임 ─────────────────────────────────
  // PH-016: 중립 → 최대 높이(L2=-1.82)까지 솟구쳤다 내려오기
  'PH-016': { name: '수직 폭발',        poses: ['P-002','P-007','P-153','P-466','P-153','P-007','P-002','P-001'], dur: 0.65 },
  // PH-017: 최고점 ↔ 중립을 빠르게 반복 (드롭 앤 팝)
  'PH-017': { name: '드롭 앤 팝',       poses: ['P-466','P-002','P-466','P-002','P-466','P-002','P-466','P-001'], dur: 0.45 },
  // PH-018: 오버헤드(L2≈-1.8) 위치에서 좌우로 흔들기
  'PH-018': { name: '오버헤드 쉐이크',  poses: ['P-153','P-156','P-160','P-157','P-153','P-156','P-160','P-153'], dur: 0.6  },

  // ── 역동 구: 상하 + 좌우 복합 ───────────────────────────────
  // PH-019: 좌/우 극단 → 양팔 최대 벌림 → 반복 (크로스 폭발)
  'PH-019': { name: '크로스 폭발',      poses: ['P-023','P-024','P-036','P-021','P-036','P-023','P-024','P-036'], dur: 0.65 },
  // PH-020: 중립 ↔ 양팔 크게 벌림을 펌핑하듯 반복
  'PH-020': { name: '파워 펌프',        poses: ['P-001','P-035','P-001','P-036','P-001','P-035','P-001','P-036'], dur: 0.5  },
};
...
// KF flat index → tlRow index 매핑 (구 블록은 여러 KF → 동일 tlRow 인덱스)
let _kfToTLRowIdx = [];   // _kfToTLRowIdx[kfIdx] = tlRowIdx
...
function nearestPoseId(t, tl) {
  ...
  if (!tl.length) return '-';
  let bestIdx = 0;
  for (let i = 0; i < tl.length; i++) { if (tl[i].time <= t) bestIdx = i; else break; }
  return tl[bestIdx].pose_id;
}

// 표시용 레이블 — 구 블록 안이면 "🎬 구이름 (n/N)" 형식
function nearestPoseLabel(t, tl) {
  if (!tl || !tl.length) return '-';
  let bestIdx = 0;
  for (let i = 0; i < tl.length; i++) { if (tl[i].time <= t) bestIdx = i; else break; }
  if (!_kfToTLRowIdx.length) return tl[bestIdx].pose_id;
  const rowIdx = _kfToTLRowIdx[bestIdx] !== undefined ? _kfToTLRowIdx[bestIdx] : bestIdx;
  const row = tlRows[rowIdx];
  if (row && row._type === 'phrase') {
    // 구 첫 번째 KF 인덱스 찾기
...
scrubber.addEventListener('input', () => {
  ...
  updateFK(safe);
  updateTimeLbl(t);
  document.getElementById('cur-pose-id').textContent = nearestPoseLabel(t, tl);
  highlightKFRow(t, tl);
...
window.applyTimeline = function() {
  ...
  function _execute() {
    ...
    // KF flat index → tlRow index 매핑 빌드 (구 블록: N개 KF → 같은 tlRow)
    _kfToTLRowIdx = [];
    tlRows.forEach((row, rowIdx) => {
      const count = row._type === 'phrase' ? row.poses.length : 1;
      for (let k = 0; k < count; k++) _kfToTLRowIdx.push(rowIdx);
    });
...
function highlightKFRow(t, tl) {
  ...
  // KF flat index 계산
  let kfIdx = 0;
  for (let i = 0; i < tl.length; i++) { if (tl[i].time <= t) kfIdx = i; else break; }
  if (kfIdx === _lastKFHighlight) return; // 동일 행 → DOM 작업 스킵
  _lastKFHighlight = kfIdx;

  // KF list (raw): kfIdx 기준 하이라이트
  document.querySelectorAll('.kf-row').forEach(r => r.classList.remove('current'));
  const kfRow = document.getElementById(`kf-row-${kfIdx}`);
  if (kfRow) { kfRow.classList.add('current'); kfRow.scrollIntoView({block:'nearest', behavior:'instant'}); }

  // 편집 행 & 비주얼 바: tlRow 인덱스로 변환 (구 블록 내 여러 KF → 같은 tlRow)
  const tlRowIdx = (_kfToTLRowIdx.length && _kfToTLRowIdx[kfIdx] !== undefined)
    ? _kfToTLRowIdx[kfIdx]
    : kfIdx;

  document.querySelectorAll('.tl-seg').forEach((s, j) => s.classList.toggle('seg-active', j === tlRowIdx));
  const editRows = document.querySelectorAll('.tl-row');
  editRows.forEach((r, j) => r.classList.toggle('preview', j === tlRowIdx));
  const activeRow = editRows[tlRowIdx];
  ...
  if (isPlaying) {
    ...
    if (!tl.length || d <= 0) {
      ...
      try {
        ...
        console.error('[animate] 재생 오류:', e, '| 타임라인:', tl.length, '개, 현재 포즈:', nearestPoseLabel(pauseOffset, tl));
        isPlaying = false;
        _syncPlayBtns();
      }
    }
